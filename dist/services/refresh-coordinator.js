...
function logBlacklistState() {
    ...
    console.log(`${FgYellow}${TAG} 📋 Blacklist state (${failedTokens.size} entries):${Reset}`);
    failedTokens.forEach((info, token) => {
        console.log(`${FgYellow}${TAG}   └─ token=${maskToken(token)} addedAt=${info.addedAt} reason="${info.reason}" status=${info.status ?? "n/a"} endpoint=${info.endpoint ?? "n/a"}${Reset}`);
    });
}
export async function getValidToken(currentRefreshToken) {
    const now = new Date().toISOString();
    console.log(`${FgCyan}${TAG} ════════════════════════════════════════${Reset}`);
    console.log(`${FgCyan}${TAG} getValidToken() called at ${now}${Reset}`);
    if (!currentRefreshToken) {
        console.warn(`${FgRed}${TAG} ❌ No refresh token provided — cannot refresh${Reset}`);
        return null;
    }
    // Token info
    const payload = parseJwtPayload(currentRefreshToken);
    const refreshRemaining = tokenRemainingSeconds(currentRefreshToken);
    console.log(`${FgGreen}${TAG} refreshToken=${maskToken(currentRefreshToken)}${Reset}`);
    console.log(`${FgGreen}${TAG} refreshToken payload: sub=${payload?.sub ?? "n/a"} iat=${payload?.iat ?? "n/a"} exp=${payload?.exp ?? "n/a"}${Reset}`);
    if (payload?.exp) {
        console.log(`${FgGreen}${TAG} refreshToken expires: ${new Date(payload.exp * 1000).toISOString()} — remaining: ${FgYellow}${fmtRemaining(refreshRemaining)}${Reset}`);
    }
    // Estado de blacklist
    logBlacklistState();
    ...
    if (blacklistEntry) {
        console.warn(`${FgRed}${TAG} 🚫 BLACKLISTED — token=${maskToken(currentRefreshToken)}${Reset}`);
        console.warn(`${FgRed}${TAG}   └─ Blacklisted at: ${blacklistEntry.addedAt}${Reset}`);
        console.warn(`${FgRed}${TAG}   └─ Reason: ${blacklistEntry.reason}${Reset}`);
        console.warn(`${FgRed}${TAG}   └─ Original status: ${blacklistEntry.status ?? "n/a"}${Reset}`);
        console.warn(`${FgRed}${TAG}   └─ Endpoint used: ${blacklistEntry.endpoint ?? "n/a"}${Reset}`);
        console.log(`${FgCyan}${TAG} ════════════════════════════════════════${Reset}`);
        return null;
    }
    ...
    if (cachedResult) {
        ...
        console.log(`${FgGreen}${TAG} 💾 CACHE HIT — this refresh token was already refreshed ${Math.round(cacheAgeMs / 1000)}s ago${Reset}`);
        console.log(`${FgGreen}${TAG}   └─ Cached accessToken: ${maskToken(cachedResult.newAccessToken)} remaining=${FgYellow}${fmtRemaining(cachedRemaining)}${Reset}`);
        if (cachedRemaining !== null && cachedRemaining > 0) {
            console.log(`${FgGreen}${TAG}   └─ Returning cached token (still valid)${Reset}`);
            console.log(`${FgCyan}${TAG} ════════════════════════════════════════${Reset}`);
            return cachedResult.newAccessToken;
        }
        else {
            console.log(`${FgYellow}${TAG}   └─ Cached token expired — removing from cache, will re-refresh${Reset}`);
    ...
    if (existingPromise) {
        console.log(`${FgYellow}${TAG} ⏳ Refresh already in-flight for this token — awaiting existing promise${Reset}`);
        return existingPromise;
    }
    // 3. Creamos la promesa única para este token
    console.log(`${FgGreen}${TAG} 🚀 No active refresh — starting new refreshSession()${Reset}`);
    const refreshPromise = (async () => {
        const startTime = Date.now();
        try {
            console.log(`${FgGreen}${TAG} Starting refreshSession for ${maskToken(currentRefreshToken)}${Reset}`);
            const result = await refreshSession(currentRefreshToken);
            const elapsed = Date.now() - startTime;
            console.log(`${FgGreen}${TAG} refreshSession completed in ${elapsed}ms — status=${result?.status} error=${result?.error ? "yes" : "no"}${Reset}`);
            if (result?.error && result?.data) {
                console.warn(`${FgRed}${TAG} refreshSession error detail: ${JSON.stringify(result.data)}${Reset}`);
            }
            if (result.error) {
                const reason = result.status === 500
                    ? `Server/network error (status=${result.status})`
                    : `Client/auth error (status=${result.status})`;
                console.error(`${FgRed}${TAG} ❌ refreshSession devolvió error — status=${result.status} elapsed=${elapsed}ms${Reset}`);
                ...
                if (result.status && result.status >= 400 && result.status < 500) {
                    ...
                    console.warn(`${FgRed}${TAG} 🚫 ADDED TO BLACKLIST: token=${maskToken(currentRefreshToken)}${Reset}`);
                    console.warn(`${FgRed}${TAG}   └─ Reason: ${reason}${Reset}`);
                    console.warn(`${FgRed}${TAG}   └─ Status: ${result.status}${Reset}`);
                    logBlacklistState();
                }
                else {
                    console.warn(`${FgYellow}${TAG} ⚠️ NOT blacklisting (status=${result.status} is not 4xx) — will retry on next request${Reset}`);
                }
                return null;
            }
            ...
            console.log(`${FgGreen}${TAG} ✅ refreshSession returned tokens present=${result.tokens ? "yes" : "no"}${Reset}`);
            if (result.tokens) {
                const newAccessRemaining = tokenRemainingSeconds(result.tokens.accessToken);
                const newRefreshRemaining = tokenRemainingSeconds(result.tokens.refreshToken);
                console.log(`${FgGreen}${TAG}   └─ new accessToken=${maskToken(result.tokens.accessToken)} remaining=${FgYellow}${fmtRemaining(newAccessRemaining)}${Reset}`);
                console.log(`${FgGreen}${TAG}   └─ new refreshToken=${maskToken(result.tokens.refreshToken)} remaining=${FgYellow}${fmtRemaining(newRefreshRemaining)}${Reset}`);
                const accessPayload = parseJwtPayload(result.tokens.accessToken);
                if (accessPayload) {
                    console.log(`${FgGreen}${TAG}   └─ accessToken iat=${accessPayload.iat ? new Date(accessPayload.iat * 1000).toISOString() : "n/a"} exp=${accessPayload.exp ? new Date(accessPayload.exp * 1000).toISOString() : "n/a"}${Reset}`);
                }
            }
            ...
            if (failedTokens.has(currentRefreshToken)) {
                ...
                console.log(`${FgGreen}${TAG} 🗑️ Removed token from blacklist after successful refresh${Reset}`);
            }
            ...
            if (newAccessToken) {
                ...
                console.log(`${FgGreen}${TAG} 💾 CACHED: old refreshToken=${maskToken(currentRefreshToken)} → new accessToken=${maskToken(newAccessToken)} (TTL=${REFRESH_CACHE_TTL_MS / 1000}s)${Reset}`);
            }
            console.log(`${FgGreen}${TAG} ✅ Token refrescado correctamente en ${elapsed}ms${Reset}`);
            console.log(`${FgCyan}${TAG} ════════════════════════════════════════${Reset}`);
            return newAccessToken;
        }
        catch (error) {
            const elapsed = Date.now() - startTime;
            console.error(`${FgRed}${TAG} ❌ EXCEPTION during refresh (${elapsed}ms): ${error instanceof Error ? error.message : String(error)}${Reset}`);
            if (error instanceof Error && error.stack) {
                console.error(`${FgGray}${TAG} Stack: ${error.stack}${Reset}`);
            }
            if (error && typeof error === "object" && "cause" in error) {
                console.error(`${FgRed}${TAG} Cause: ${String(error.cause)}${Reset}`);
            }
            console.log(`${FgCyan}${TAG} ════════════════════════════════════════${Reset}`);
            return null;
        }
...
//# sourceMappingURL=refresh-coordinator.js.map