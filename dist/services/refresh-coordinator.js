...
export async function getValidToken(currentRefreshToken) {
    ...
    try {
        ...
        console.log(`${FgGreen}[refresh-coordinator] getValidToken called for refreshToken=${maskToken(currentRefreshToken)} payload=${payload
            ? JSON.stringify({
                sub: payload.sub,
                exp: payload.exp,
                iat: payload.iat,
        ...
        if (payload?.exp) {
            try {
                console.log(`${FgGreen}[refresh-coordinator] refreshToken exp: ${new Date(payload.exp * 1000).toISOString()} (${payload.exp})${Reset}`);
            }
            catch { }
        }
    }
    catch (err) {
        console.warn(`${FgRed}[refresh-coordinator] Error decoding refresh token: ${err}${Reset}`);
    }
    ...
    if (failedTokens.has(currentRefreshToken)) {
        console.warn(`${FgRed}[refresh-coordinator] refreshToken is blacklisted (failed before): ${maskToken(currentRefreshToken)}${Reset}`);
        return null;
    }
    ...
    const refreshPromise = (async () => {
        try {
            console.log(`${FgGreen}[refresh-coordinator] Starting refreshSession for ${maskToken(currentRefreshToken)}${Reset}`);
            const result = await refreshSession(currentRefreshToken);
            console.log(`${FgGreen}[refresh-coordinator] refreshSession result status=${result?.status} error=${result?.error ? "yes" : "no"}${Reset}`);
            if (result?.error && result?.data) {
                console.warn(`${FgRed}[refresh-coordinator] refreshSession detail: ${JSON.stringify(result.data)}${Reset}`);
            }
            if (result.error) {
                console.error(FgRed +
                ...
                if (result.status && result.status >= 400 && result.status < 500) {
                    ...
                    console.warn(`${FgRed}[refresh-coordinator] Marking refresh token as failed (no-retry): ${maskToken(currentRefreshToken)}${Reset}`);
                }
                return null;
            }
            ...
            try {
                ...
                if (session?.tokens) {
                    ...
                    if (accessPayload?.iat) {
                        console.log(`${FgGreen}[refresh-coordinator] accessToken iat: ${new Date(accessPayload.iat * 1000).toISOString()} (${accessPayload.iat})${Reset}`);
                    }
                }
            }
            catch (err) {
                console.warn(`${FgRed}[refresh-coordinator] Error logging session tokens: ${err}${Reset}`);
            }
