...
function normalizeUrl(url) {
    if (!url)
        return url;
    if (url === "/")
        return "/"; // Mantener raíz si se diera el caso
...
export function initSSO(options) {
    ...
    if (options.ssoUrl)
        config.NEXT_PUBLIC_SSO_URL = normalizeUrl(options.ssoUrl);
    ...
    if (options.cookieName)
        config.COOKIE_SESSION_NAME = options.cookieName;
    ...
    if (options.endpoints) {
        ...
        const normalizedOverrides = Object.fromEntries(Object.entries(options.endpoints).map(([k, v]) => [
            k,
